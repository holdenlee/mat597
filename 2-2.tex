...
\begin{df} The \ivocab{equidistribution assumption}. \\
...
\approx e^{Vs(\rh)}
\]
where density $\rho = m/V$ and $s(p) = -p\ln(p) - (1- p)\ln(1 - p)$. $\rho$ varies between $0$ and $1$. $s(\rho)$ is concave. This function attains maximum of $\ln 2$ at $\rc2$ where it has quadratic behavior.
\end{lem}
\begin{proof}
...
\end{align*}
where $C$ is a normalizing constant. The probability is propotional in exponent to the volume of the complement multiplied times the entropy of the density, which we saw in the exercise above. Then we re-write the density of the small box in terms of the overall density: 
\bal
