...
\be
\mathbbm{1}[\text{cond}] := \begin{cases}
...
\be
\mathbb{P}(n_{\Omega})= \frac{\mathbbm{1}[\sum_{x\in \Omega}n_x=N]}{Z}
\ee
...
\be
\left\langle {f}\right\rangle_{N,n}^{\text{Can}} = \frac{\sum_{n\in \Omega} \mathbbm{1}[n_x=N] f(n)}{\sum_{n\in \Omega} \mathbbm{1}[\sum n_x=N]}.
\ee
\end{definition}
...
\be\frac{\partial}{\partial h}\txtn{ln}(Z_{\Lambda}) = \frac{\beta}{|\Lambda|} \int (\sum_{x \in \Lambda} \sigma_x) \frac{e^{-\beta H_{\Lambda}(\sigma)}}{Z} \rho_0(d\sigma_{\Lambda}).\ee 
